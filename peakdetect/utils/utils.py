...
def plot_bbox(imgs, targets, classes):
    imgs = imgs.to("cpu")
    targets = targets.to("cpu")

    fig, axs = plt.subplots(1,2, figsize=(3,3), dpi=150)
    axs[0].imshow(imgs.squeeze().numpy())

    axs[1].imshow(imgs.squeeze().numpy())
    detections = targets
...
# img, targets,_,_ = data_module.train.__getitem__(0)
# targets[:, 2:] = xywh2xyxy(targets[:, 2:])
# targets[:, 2:] *= imgs.size(3)
# plot_bbox(img, targets, model.class_names)


def create_train_valid_angle_files(path):
    with open(path,'r') as f:
        lines=f.readlines()
        head = lines[:2]
        lines=lines[2:]
        line_train, line_test= train_test_split(lines, test_size=0.05, random_state=0)

    with open(path.split('.')[0]+'_train.txt','w') as f:
        for line in head:
            f.write(line)
        for line in line_train:
            f.write(line)

    with open(path.split('.')[0]+'_valid.txt','w') as f:
        for line in head:
            f.write(line)
...
def get_batch_statistics(outputs, targets, iou_threshold):
    ...
    for sample_i in range(len(outputs)):
        ...
        output = outputs[sample_i].type_as(targets)
        pred_boxes = output[:, :4]
